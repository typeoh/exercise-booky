...
[
  {
    it:
      "Heaven sends an informative message to Hell " +
      "and gets feedback from there via purgatory",
    message: "here I am"
  },
  {
    it:
      "Heaven sends a question to Hell " +
      "and gets feedback from there via purgatory",
  ...
  it(scenario.it, () => {
    ...
    const heaven = mount(<Heaven message={message} onSubmit={onSubmit} />);
    ...
    //then
    expect(getOutFromHellStub).to.have.been.calledWith(
      `Yes, ${message} indeed!`
    );
    expect(getOutFromPurgatoryStub).to.have.been.calledWith(
      `Yes, ${message} indeed!!`
    );
    expect(getOutFromHeavenStub).to.have.been.calledWith(
      `Yes, ${message} indeed!!!`
    );
    expect(onSubmit).to.have.been.calledWith(`Yes, ${message} indeed!!!`);
